...
# if (sim_beds) {
    data_dir <- paste0("~/SSA_model/CNN/pilot/training_data_bed/", setsf)
...
ssa_steady <- readRDS(file = paste("~/SSA_model/CNN/pilot/training_data_bed/initial_conds/ssa_steady_20220329.rds", sep = ""))
...
# if (sim_beds) {
    plot_dir <- paste0("./plots/sim_beds/", output_var, "/", setsf)
# } else {
...
if (save_plots) {
    ...
    png(paste0(plot_dir, "/pred_vs_true_bed", checkpt, ".png"), width = 1000, height = 500)
    par(mfrow = c(length(samples) / 2, 2))
    ...
    png(paste0(plot_dir, "/pred_vs_true_fric", checkpt, ".png"), width = 1000, height = 500)
    par(mfrow = c(length(samples) / 2, 2))
