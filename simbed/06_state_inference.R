...
s <- 1 # sample(1:n_test_samples, 1) # sample index
fric_s <- pred_fric[, s]
bed_s <- pred_bed[, s]
...
if (run_EnKF) {
  ...
  enkf_thickness <- readRDS(file = paste0(output_dir, "/enkf_thickness_", output_date, ".rds", sep = ""))
  enkf_bed <- readRDS(file = paste0(output_dir, "/enkf_bed_", output_date, ".rds", sep = ""))
  enkf_friction <- readRDS(file = paste0(output_dir, "/enkf_friction_", output_date, ".rds", sep = ""))
  enkf_velocities <- readRDS(file = paste0(output_dir, "/enkf_velocities_", output_date, ".rds", sep = ""))
}
...
if (plot_ice_thickness) {
  ...
  for (t in plot_times) {
    ...
    png(paste0(plot_dir, "/test_ice_thickness_", t - 1, ".png"), width = 2000, height = 1000, res = 300)
    print(thickness_plot)
    dev.off()
  }
...
if (plot_velocity) {
  for (t in plot_times) {
    ...
    png(paste0(plot_dir, "/test_velocity_", t - 1, ".png"), width = 2000, height = 1000, res = 300)
    print(velocity_plot)
    dev.off()
  }
...
enkf.rmse <- c()
for (t in plot_times) {
  enkf.rmse[t] <- rmse(rowMeans(enkf_thickness[[t]]), true_thicknesses[s, , t])
