...
test_samples <- seq(100, 500, 100) # sample(1:n_test_samples, 1) # sample index
...
for (s in test_samples) {
  ...
  if (run_EnKF) {
    ...
    enkf_out <- run_enkf(domain = domain, years = years, 
    ...
    if (save_enkf_output) {
      ...
      saveRDS(enkf_thickness, file = paste0(output_dir, "/enkf_thickness_sample_", s, "_", output_date, ".rds", sep = ""))
      saveRDS(enkf_bed, file = paste0(output_dir, "/enkf_bed_sample_", s, "_", output_date, ".rds", sep = ""))
      saveRDS(enkf_friction, file = paste0(output_dir, "/enkf_friction_sample_", s, "_", output_date, ".rds", sep = ""))
      saveRDS(enkf_velocities, file = paste0(output_dir, "/enkf_velocities_sample_", s, "_", output_date, ".rds", sep = ""))
    }
  } else {
    enkf_thickness <- readRDS(file = paste0(output_dir, "/enkf_thickness_sample_", s, "_", output_date, ".rds", sep = ""))
    enkf_bed <- readRDS(file = paste0(output_dir, "/enkf_bed_sample_", s, "_", output_date, ".rds", sep = ""))
    enkf_friction <- readRDS(file = paste0(output_dir, "/enkf_friction_sample_", s, "_", output_date, ".rds", sep = ""))
    enkf_velocities <- readRDS(file = paste0(output_dir, "/enkf_velocities_sample_", s, "_", output_date, ".rds", sep = ""))
  }
  ...
  plot_dir <- paste0("./plots/all/", setsf, "/sample", s)
