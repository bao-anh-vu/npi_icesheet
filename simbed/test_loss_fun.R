...
## Generate some Lb, Lc, Lg matrices (Cholesky factors for the bed, friction and GL respectively) 
...
y_true <- rep(1, d) # this is basically "theta"
y_pred <- c(rep(0, d), log(diag(Lb)), diag(Lb[-1, ]), 
                        log(diag(Lc)), diag(Lc[-1, ]), 
                        log(diag(Lg)), diag(Lg[-1, ])) # and this is to mimic the "output layer" of the NN

# Loss function
test_loss <- posterior_loss_wrap(n_basis_funs, n_gl)(y_true, y_pred) 

# Value of Gau(y_true; mean, cov), 
Linv <- as.matrix(solve(L))
true_loss <- dmvnorm(y_true, mean = y_pred[1:d],
                    sigma = t(Linv) %*% Linv, log = T) + d/2 * log(2*pi)
## need to add that d/2 * log(2*pi) term because it's a constant term that I didn't include in the loss function
...
## Compute loss function for ONE pair of (y_true, y_pred)
...
# this should return the same value as test_loss and true_loss

## Now compute the loss function for a batch (just replicating y_true and y_pred multiple times)
batch_size <- 64L
...
test_loss_tf_batch <- posterior_loss_wrap_tf(n_basis_funs, n_gl)(y_true_batch, y_pred_batch) # - d/2 * log(2*pi)
