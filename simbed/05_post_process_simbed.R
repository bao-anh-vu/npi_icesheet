...
model_quantile <- create_model_quantile(input_dim = input_dim, output_dim = output_dim, quantile = 0.95)
...
if (!is.null(checkpt_quantile)) {
    ## Load the model from checkpt
    cp_restart_quantile <- paste0(output_dir_quantile, "/checkpoints/cp-", formatC(checkpt, width = 4, flag = "0"), ".ckpt")
    # latest <- tf$train$latest_checkpoint(checkpoint_dir)
...
## Predict
pred_output <- model_quantile %>% predict(test_input)
...
# if (sim_beds) {
    if (output_var == "bed") {
        ...
        pred_bed_demean <- basis_mat %*% t(pred_output)
        test_bed_demean <- basis_mat %*% t(test_output)
...
samples <- sample(1:nrow(test_output), 2)
