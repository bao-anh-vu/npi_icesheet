### Fix the parameter estimate at the posterior mean ###
### And run state inference with the EnKF ###
...
## Presets
data_date <- "20240320" # data date here is the date of the output from the CNN
output_date <- "20250708" # "20240518" # output of the EnKF run 
...
## Command line args
args <- c(1, 1) #commandArgs(trailingOnly = TRUE)
if (length(args) < 2) {
...
# for (s in test_samples) {
    ...
    if (use_missing_pattern) {
        enkf_output_dir <- paste0("./output/enkf_postmean/", setsf)
    } else {
        enkf_output_dir <- paste0("./output/enkf_postmean/", setsf)
    }
    ...
    if (run_EnKF) {
        ...
        bed_sample <- pred_bed[, s]
        fric_sample <- pred_fric[, s]
        # ini_ens_list <- list()
        # for (p in 1:n_params) {
            ...
            if (regenerate_ini_ens) {
                ...
                if (save_enkf_output) {
                    qsave(ini_ens, file = paste0(enkf_output_dir, "/ini_ens_", output_date, ".qs", sep = ""))
                }
            } 
        # }
    } else {
        ini_ens <- qread(file = paste0(enkf_output_dir, "/ini_ens_", output_date, ".qs", sep = ""))
    ...
    if (run_EnKF) {
        ...
        if (save_enkf_output) {
            qsave(enkf_thickness, file = paste0(enkf_output_dir, "/enkf_thickness", sample_ind, "_Ne", Ne, "_", output_date, ".qs", sep = ""))
            qsave(enkf_bed, file = paste0(enkf_output_dir, "/enkf_bed_sample", sample_ind, "_Ne", Ne, "_", output_date, ".qs", sep = ""))
            qsave(enkf_friction, file = paste0(enkf_output_dir, "/enkf_friction_sample", sample_ind, "_Ne", Ne, "_", output_date, ".qs", sep = ""))
            qsave(enkf_velocities, file = paste0(enkf_output_dir, "/enkf_velocities_sample", sample_ind, "_Ne", Ne, "_", output_date, ".qs", sep = ""))
            # qsave(error_inds, file = paste0(enkf_output_dir, "/error_inds_sample", s, "_Ne", Ne, "_", output_date, "_", run, ".qs", sep = ""))
        }
    } else {
        enkf_thickness <- qread(file = paste0(enkf_output_dir, "/enkf_thickness_sample", sample_ind, "_Ne", Ne, "_", output_date, ".qs", sep = ""))
        enkf_bed <- qread(file = paste0(enkf_output_dir, "/enkf_bed_sample", sample_ind, "_Ne", Ne, "_", output_date, ".qs", sep = ""))
        enkf_friction <- qread(file = paste0(enkf_output_dir, "/enkf_friction_sample", sample_ind, "_Ne", Ne, "_", output_date, ".qs", sep = ""))
        enkf_velocities <- qread(file = paste0(enkf_output_dir, "/enkf_velocities_sample", sample_ind, "_Ne", Ne, "_", output_date, ".qs", sep = ""))
    }
    ...
    if (use_missing_pattern) {
        plot_dir <- paste0("./plots/enkf_postmean/", setsf, "/missing/sample", sample_ind, "/")
    } else {
        plot_dir <- paste0("./plots/enkf_postmean/", setsf, "/nonmissing/sample", sample_ind, "/")
    }
    ...
    if (plot_friction) {
        ...
        matplot(domain[plot_region] / 1000, exp(enkf_friction[plot_region, ]),
            type = "l", col = "lightpink",
        ...
        lines(domain[plot_region] / 1000, rowMeans(exp(enkf_friction[plot_region, ])), col = "red", lwd = 1.5)
    ...
    for (i in 1:length(save_points)) {
        t <- save_points[i]
        enkf.rmse[i] <- rmse(rowMeans(enkf_thickness[[i]]), true_thicknesses[s, , t])
    }
