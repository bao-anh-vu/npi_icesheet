...
run_enkf_missing <- function(domain, years, steps_per_yr, ini_thickness, ini_bed, 
                     ...
                     run_analysis = TRUE, use_cov_taper = FALSE,
                     inflate_cov = FALSE, 
...
# png(paste0("./plots/temp/ini_thickness.png"))
...
png(paste0("./plots/temp/velocity_ens_", year, "_s10.png"))
...
run_enkf_missing_noH <- function(domain, years, steps_per_yr, ini_thickness, ini_bed, 
                     ini_friction_coef, ini_velocity, observations, 
                     missing_pattern = NULL, run_analysis = TRUE, 
                     use_cov_taper = F,
                     inflate_cov = F, 
                     add_process_noise = TRUE, process_noise_info,
                     use_const_measure_error = FALSE,
  ...
  if (!is.null(missing_pattern)) { # turn this part into a function later
    ...
    ## Construct the "missing observation matrix" C_t here
      ...
      C_t_ls <- lapply(1:(years+1), function(y) {
        as(bdiag(mp_surface_elev_ls[[y]], mp_vel_ls[[y]]), "CsparseMatrix")
      })

      # C_t <- as.matrix(mp_vel_ls[[1]]) #as.matrix(C_t_ls[[1]])
      # png("./plots/temp/C_t.png")
      # image(t(C_t[nrow(C_t):1,]), col = c("white", "black")) # to orient it so row 1 is on top
      # dev.off()
  } else {
  ...
  # State ensemble 
  enkf_ens <- list()
  ens <- rbind(ini_thickness, ini_bed, ini_friction_coef)
  enkf_ens[[1]] <- ens
  enkf_means <- matrix(NA, nrow = 3*J, ncol = years + 1)
  enkf_means[, 1] <- rowMeans(ens)
  ...
  for (year in 2:(years+1)) {
    ...
    if (add_process_noise) { ## Add process noise
      ...
      # ens_all <- rbind(ens, ens_all[-(1:J), ]) # append velocity to ensemble
    }

      # if (inflate_cov) {
      # # print("Inflating covariance...")
      #   ens_all[1:(3*J), ] <- ens_all[1:(3*J), ] + 0.1 * (ens_all[1:(3*J), ] - rowMeans(ens_all[1:(3*J), ]))
      # }
    ...
    if (run_analysis) {
      ...
      surf_elev_noise_sd  <- rep(10, J)
      # vel_noise_sd <- pmin(0.25 * rowMeans(prev_velocity), 20) #pmin(0.25 * vel_obs, 20)
      # vel_noise_sd[vel_noise_sd <= 0] <- 1 #1e-05
