get_obs <- function(reference, data_date, n_obs = 50, get_new_obs = FALSE, rewrite_obs = FALSE) {
  ...
  if (get_new_obs) {
    ...
    noise <- rnorm(n_surface_obs, rep(0, n_surface_obs), rep(10, n_surface_obs))
    
    obs_surface <- all_top_surface + matrix(noise, nrow = nrow(all_top_surface), ncol = ncol(all_top_surface))
    ...
    obs_list <- list(velocity_obs = obs_velocities,
                     bed_obs = obs_bed,
                     bed_obs_locations = obs_ind,
                     surface_obs = obs_surface)
    ...
    obs_list <- readRDS(file = paste("/home/babv971/SSA_model/EnKF/Output/observations_20220318.rds", sep = ""))
  }
