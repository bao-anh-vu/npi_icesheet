run_enkf <- function(domain, years, steps_per_yr, ini_ens, ini_bed, 
                     ini_friction_coef, ini_velocity, observations,   
                     run_analysis = TRUE, use_cov_taper = TRUE, add_process_noise = TRUE) {
  
  print("Running filter...")
  
  Ne <- ncol(ini_ens) # Ensemble size
  J <- length(domain)
  ...
  for (year in 2:(years+1)) {
    ...
    if (add_process_noise) { ## Add process noise
      ...
      h_noise <- cond_sim(mu = rep(0, J), sd = h_sd,
                          domain, l = 100e3, nsims = Ne)
      
      ens <- as.matrix(ens + h_noise[, 1:Ne]) # add noise to ice thickness
    ...
    if (run_analysis) {
      ...
      y <- c(observations$surface_obs[, year], observations$velocity_obs[, year])
