initialise_ice_thickness <- function(domain, surface_obs, beds, 
                                     n_sims,
                                     rho = 910, rho_w = 1028, 
                                     condsim_shelf = FALSE) {
  ...
  simulated_thickness <- matrix(0, nrow = J, ncol = ncol(beds))
  thickness_ens <- list()
  
  for (i in 1:ncol(beds)) {
    h <- surface_obs - beds[, i]
    
    ## Calculate GL position for each ensemble member
    GL <- gl_migrate2(surface_obs, beds[, i]) #s[, i])
    # cat("GL: ", GL, "\n")
    ...
    if (condsim_shelf) { # use conditional simulation on the ice shelf only
      ...
      h.sd <- c(rep(30, GL), rep(20, length(h) - GL))
