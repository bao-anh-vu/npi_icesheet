...
create_ref <- function(use_stored_steady_state = TRUE, 
  ...
  if (!use_stored_steady_state) {
    ...
    if (rewrite_steady_state) {
      ...
      saveRDS(ssa_out, file = paste("/home/babv971/SSA_model/EnKF/Output/ssa_steady_", data_date, ".rds", sep = ""))
    }
    
  } else {
    
    ## Read output from saved file
    # ssa_steady <- readRDS(file = paste("/home/babv971/SSA_model/EnKF/Output/ssa_steady_", data_date, ".rds", sep = ""))
  ...
  if (!use_stored_reference) {
    ...
    reference <- readRDS(file = paste("/home/babv971/SSA_model/EnKF/Output/reference_20220318.rds", sep = ""))
