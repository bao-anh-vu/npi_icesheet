## Posterior predictive distribution for test data example
...
test_sim <- 10
test_input <- test_data$input[test_sim, , ,]

## Scale back the input
test_se <- test_input[,,1] * test_data$input_sd[1] + test_data$input_mean[1]
test_vel <- test_input[,,2] * test_data$input_sd[2] + test_data$input_mean[2]
...
fric_post_means <- qread(file = paste0(output_dir, "/pred_fric_", data_date, ".qs"))
bed_post_means <- qread(file = paste0(output_dir, "/pred_bed_", data_date, ".qs"))
...
post_means <- list()
post_means[[1]] <- list(
    friction = as.vector(fric_post_means[, 1]), #* 1e6 * params$secpera^(1 / params$n),
    bedrock = as.vector(bed_post_means[, 1]) #+ bed_mean
)

true_params <- list()
true_params[[1]] <- list(
    friction = test_data$true_fric[test_sim, ], #* 1e6 * params$secpera^(1 / params$n),
    bedrock = test_data$true_bed[test_sim, ] #+ bed_mean
)

## Plot parameters
# png(filename = paste0(plot_dir, "/fric_bed_samples_", data_date, ".png"), width = 800, height = 400)
# par(mfrow = c(2, 1))
...
sim_results <- sim_obs(
        param_list = true_params, # post_means, #param_list,
        domain = domain,
        phys_params = params,
        years = 10, # sim_beds = T,
        # warmup = warmup,
...
# s <- 1
png(filename = paste0(plot_dir, "/pred_post_on_test_", data_date, ".png"), width = 800, height = 800)
par(mfrow = c(2, 1))
matplot(surface_obs_arr[1,,,1], type = 'l', lty = 1, col = rgb(0,0,0,0.3), 
main = "Simulated surface elevation", ylab = "Surface elevation (m)", xlab = "Domain")
matlines(test_se, col = 'salmon')
legend("topright", legend = c("Predicted", "True"), col = c(rgb(0,0,0,0.3), 'salmon'), lty = 1)

matplot(surface_obs_arr[1,,,2], type = 'l', lty = 1, col = rgb(0,0,0,0.3),
main = "Simulated surface velocity", ylab = "Surface velocity (m/yr)", xlab = "Domain")
# matlines(velocity_data, col = 'salmon')
matlines(test_vel, col = 'salmon')
legend("topright", legend = c("Predicted", "True"), col = c(rgb(0,0,0,0.3), 'salmon'), lty = 1)
dev.off()
...
# png(filename = paste0(plot_dir, "/pred_adj_on_test_", data_date, ".png"), width = 800, height = 800)
# par(mfrow = c(2, 1))
