...
years <- max(time_cf$year) - min(time_cf$year)
annual_height_change_re2014 <- mclapply(0:years, function(i) { ## annual height change relative to 2014
...
for (t in 1:years) {
    varname <- paste0("height_change_", t)
...
## SMB
annual_smb <- mclapply(0:years, function(i) {
...
## Melt rate
annual_melt <- mclapply(0:years, function(i) {
    melt_t <- melt[,,(i*4+1) : ((i+1)*4)]
    melt_avg <- apply(melt_t, c(1, 2), mean, na.rm = TRUE)
}, mc.cores = 10L)

melt_df <- data.frame(expand.grid(x = x, y = y))

for (t in 1:(years+1)) {
    varname <- paste0("melt_", t)
