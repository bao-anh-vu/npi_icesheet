...
## BedMachined data for comparison
...
if (censor_data) { # reserve some data points for validation
        ...
        bed_obs_df$chosen <- 1 ## choose all observations
        bed_obs_chosen <- bed_obs_df
}
...
# bed.nugget <- 0 #200
n_sims <- 10
bed_sim_output <- simulate_bed(n_sims,
...
bed_sim_plot <- bed_sim_df %>% ggplot(aes(x = loc, y = bed_elev)) +
        ...
        geom_line(data = data.frame(x = flowline_dist, y = bedmachine), aes(x = x, y = y), col = "blue", lwd = 1) +
