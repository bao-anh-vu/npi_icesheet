...
run_enkf_missing <- function(domain,
                             years, steps_per_yr,
                             phys_params,
                             #  ini_thickness, ini_bed, ini_friction_coef,
                             ini_ens,
                             ini_velocity,
                             observations,
  ...
  if (!is.null(missing_pattern)) { # turn this part into a function later
    ...
    # })
  } else { # no missing data, use identity matrix
  ...
  for (year in 1:years) {
    ...
    par(mfrow = c(2, 1))
    matplot(ens_all[1:J, ], type = "l", main = "Thickness ensemble")

    matplot(ens_all[(3 * J + 1):(4 * J), ], type = "l", main = "Velocity ensemble")
    dev.off()
    ...
    if (run_analysis) {
      ...
      surf_elev_noise_sd <- rep(5, J)

      if (use_const_measure_error) {
        vel_noise_sd <- rep(20, J)
      } else {
        vel_noise_sd <- pmin(0.25 * rowMeans(prev_velocity), 50) # pmin(0.25 * vel_obs, 20)
      ...
      noise <- matrix(rnorm(length(diag(R)) * Ne, mean = 0, sd = sqrt(diag(R))), ncol = Ne)
      ...
      if (use_cov_taper) {
        ...
        png("./plots/temp/Y_HX.png", width = 800, height = 600)
        matplot(Y, type = "l", ylim = c(0, 1500))
        matlines(HX + noise, col = "salmon")
        dev.off()
      ...
      ##### IV. Update velocity based on the updated thickness #####
      ...
      velocity.list <- mclapply(ens.list,
        function(v) {
          u <- as.vector(solve_velocity(
            ...
            domain = domain,
            bed = v[(J + 1):(2 * J)],
            friction = exp(v[(2 * J + 1):(3 * J)]) * fric_scale,
            phys_params = phys_params
          ))
    ...
    enkf_velocity_means[, year] <- rowMeans(prev_velocity)
  }
