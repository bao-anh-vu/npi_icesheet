...
#     concat_fric <- do.call("cbind", friction_list)
#     # friction_arr <- array(concat_fric, dim = c(N, length(friction_list[[1]])))
#     friction_arr <- t(concat_fric)

#     # if (sim_beds) {
#         concat_bed <- do.call("cbind", bed_list)
#         # bed_arr <- array(concat_bed, dim = c(N, length(bed_list[[1]])))
#         bed_arr <- t(concat_bed)
#     # }

#     concat_gl <- do.call("cbind", gl_list)
#     # gl_arr <- array(concat_gl, dim = c(N, length(gl_list[[1]])))
#     gl_arr <- t(concat_gl)
