    ...
    tryCatch(
        {
            ...
            tf$config$experimental$set_virtual_device_configuration(
                gpus[[1]],
                list(tf$config$experimental$VirtualDeviceConfiguration(memory_limit = 4096 * 5))
            )
...
for (quantile in quantiles) {
    ...
    output_dir <- paste0("./output/neural_bayes/", setsf, "/missing/q", q_string, "/")
    # } else {
    ...
    if (rerun_cnn) {
        ...
        save_model_tf(model, paste0(output_dir, "model", data_date))
    } else {
        model <- load_model_tf(paste0(output_dir, "model", data_date))
        history <- qread(file = paste0(output_dir, "history", data_date, ".qs"))
    }
    ...
    png(paste0("./plots/neural_bayes/", setsf, "/loss_", q_string, ".png"))
    print(loss_plot)
    dev.off()
}
