...
## Leave one year out for forecasting later
if (leave_one_out) {
    n_years <- dim(surface_obs_arr)[3] - 1
    print(paste0("Using first ", n_years, " years of data for leave-one-out cross-validation"))
} 

## Calculate mean and sd over space and time for surface elevation and velocity separately
surf_elev_mean <- mean(surface_obs_arr_train[n_for_mean, , 1:n_years, 1]) 
velocity_mean <- mean(surface_obs_arr_train[n_for_mean, , 1:n_years, 2])
surf_elev_sd <- sd(surface_obs_arr_train[n_for_mean, , 1:n_years, 1])
velocity_sd <- sd(surface_obs_arr_train[n_for_mean, , 1:n_years, 2])
