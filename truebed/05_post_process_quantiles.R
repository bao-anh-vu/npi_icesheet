...
if (output_var == "all") {
    ...
    pred_gl <- pred_output[, (n_fric_basis+1):ncol(pred_output)] * test_data$sd_gl + test_data$mean_gl
    ...
    test_gl <- test_output[, (n_fric_basis+1):ncol(pred_output)] * test_data$sd_gl + test_data$mean_gl
...
for (q in 1:length(quantiles)) {
    ...
    model_quantile <- create_model_quantile(input_dim = input_dim, output_dim = output_dim, quantile = q)
    ...
    pred_gl_quantile <- pred_output_quantile[, (n_fric_basis+1):ncol(pred_output_quantile)] * test_data$sd_gl + test_data$mean_gl
    ...
    pred_quantiles[[q]] <- list(fric = pred_fric_quantile, gl = pred_gl_quantile)
}
...
#         lines(domain[plot_domain] / 1000, pred_bed_lq[plot_domain, i], lty = 3, lwd = 2, col = "salmon")
#         lines(domain[plot_domain] / 1000, pred_bed_uq[plot_domain, i], lty = 3, lwd = 2, col = "salmon")
...
# if (output_var == "friction" | output_var == "all") {
    ...
    for (i in 1:length(samples)) {
        ...
        lines(domain[plot_domain] / 1000, pred_fric_lq[plot_domain, i], lty = 3, lwd = 2, col = "lightblue")
...
if (output_var == "friction") {## Post-process output from CNN
   ...
   pred <- pred_gl
    ...
    truth <- cbind(test_fric, test_gl)
    pred <- cbind(pred_fric, pred_gl)
} else {
    print("Output variable should be either 'friction', 'bed' or 'grounding_line'.")
}
