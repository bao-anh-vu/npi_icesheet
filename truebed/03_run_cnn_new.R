...
if (output_var == "friction") {
  ...
  train_output <- cbind(train_data$fric_coefs, train_data$grounding_line)
  val_output <- cbind(val_data$fric_coefs, val_data$grounding_line)
  test_output <- cbind(test_data$fric_coefs, test_data$grounding_line)
}
