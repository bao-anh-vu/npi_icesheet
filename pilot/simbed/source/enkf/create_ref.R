...
create_ref <- function(use_stored_steady_state = TRUE, 
  ...
  if (!use_stored_steady_state) {
    ...
    if (rewrite_steady_state) {
      ...
      saveRDS(ssa_out, file = paste("./training_data/initial_conds/ssa_steady_", data_date, ".rds", sep = ""))
    }
    
  } else {
    
    ## Read output from saved file
    ssa_steady <- readRDS(file = paste("./training_data/initial_conds/ssa_steady_", data_date, ".rds", sep = ""))
  ...
  if (!use_stored_reference) {
    ...
    if (rewrite_reference) {
      ...
      saveRDS(reference, file = paste("./training_data/initial_conds/reference_", data_date, ".rds", sep = ""))
    }
    
  } else {
    # Restore data form a saved file
    # reference <- readRDS(file = "assim_data.rds")
    reference <- readRDS(file = paste("./training_data/initial_conds/reference_", data_date, ".rds", sep = ""))
