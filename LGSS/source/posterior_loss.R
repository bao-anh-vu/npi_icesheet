...
# y_true <- rnorm(3)
# d <- 3
# y_pred <- c(rnorm(3), rnorm(5))
# mean <- y_pred[1:d]
# chol_vals <- y_pred[(d+1):length(y_pred)]
# L_true <- diag(exp(chol_vals[1:3]))
# diag(L_true[-1, ]) <- chol_vals[-(1:3)]

# inv_L_true <- solve(L_true)
# Sigma_true <- t(inv_L_true) %*% inv_L_true
# p_true <- dmvnorm(y_true, mean = mean, sigma = solve(L_true %*% t(L_true)), log = T)

# ## Loss from tf
# # chol_vals_tf <- tf$constant(1:5)
# batchsize <- 2L
# chol_vals_mat <- matrix(rep(chol_vals, batchsize), nrow = batchsize, byrow = T)
# chol_vals_mat_tf <- tf$constant(chol_vals_mat, dtype = tf$float64)

# test_L <- construct_L_matrix_tf(chol_vals_mat_tf, 3L)
# mean_tf <- tf$constant(mean, dtype = tf$float64)

# y_pred_mat <- matrix(rep(y_pred, batchsize), nrow = batchsize, byrow = T)
# y_pred_mat_tf <- tf$constant(y_pred_mat, dtype = tf$float64)
# y_true_mat <- matrix(rep(y_true, batchsize), nrow = batchsize, byrow = T)
# y_true_mat_tf <- tf$constant(y_true_mat, dtype = tf$float64)

# loss_fun <- posterior_loss_wrap_tf(3)#(y_true = y_true_mat_tf, y_pred = chol_vals_mat_tf)
# loss_fun2 <- loss_fun(y_true_mat_tf, y_pred_mat_tf)
# tf_loss <- as.vector(loss_fun2) #+ d/2 * log(2 * pi) 
