...
construct_L_mat <- function(v, d) {
  ...
  L_mat[lower.tri(L_mat, diag = F)] <- v[(d + 1):length(v)]
...
for (s in 1:nrow(test_output)) {
    ...
    sigma_eps_samples <- sqrt(exp(post_samples[, 2]))
...
sigma_eps_true <- sqrt(exp(test_output[, 2]))
...
# print(mean_plot)
grid.arrange(phi_mean_plot, sigma_eps_mean_plot, ncol = 2)
dev.off()
...
grid.arrange(phi_intv_plot, sigma_eps_intv_plot, nrow = 2)
dev.off()
...
for (s in test_sample) {
  ...
  hmc.sigma_eps_samples[[s]] <- hmc_output$draws[, , 2]
}

png(paste0("./plots/lgss_hmc_vs_cnn_", data_date, ".png"), width = 600, height = 1200)
par(mfrow = c(length(test_sample), 2))
