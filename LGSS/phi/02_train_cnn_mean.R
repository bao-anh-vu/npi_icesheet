...
png(paste0("./output_mean/loss_plot_", data_date, ".png"))
plot(history)
dev.off()
...
pred_mean <- pred_output

# Compare predicted mean vs true parameters for a chosen sample in the test set
phi_pred <- tanh(pred_mean) # back-transform
phi_true <- tanh(test_output)
mean_pred_df <- data.frame(pred = phi_pred, true = phi_true)

png(paste0(output_dir, "/pred_vs_test_plot_", data_date, ".png"), width = 500, height = 500)

ggplot(mean_pred_df, aes(x = true, y = pred)) + 
  geom_point() + 
